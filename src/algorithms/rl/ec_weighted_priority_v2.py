...
class EnergyCommunityWeightedPriorityV2(MultiAgentEnv):
    """
    Based on a weighted priority, this environment is designed to
    simulate an energy community with multiple agents.
    Reward is given only when a day ends
    """

    metadata = {'name': 'EnergyCommunityWeightedPriority-v2'}
    ...
    def _reset(self):
        ...
        if self.timestep == 0:
            ...
            self.priority_system = WeightedPriorityV0(self.storages + self.evs,
                                                      urgency_weight=4.0,
                                                      capacity_weight=0.4,
                                                      log_scaling=False)
    ...
    def create_contribution_dict(self):
        ...
        for res in self.storages:
            res_value = res.value[self.timestep - 1] if self.timestep > 0 else res.initial_charge

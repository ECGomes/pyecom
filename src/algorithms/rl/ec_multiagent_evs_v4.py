...
class EVMultiAgent_v4(MultiAgentEnv):
    """
    ...
    - 'action_indicator': whether the EV is charging or discharging. 0: charging, 1: discharging
    """
    ...
    def __init__(self, resources: list[Vehicle],
        ...
        self.evs = deepcopy(resources)

        # Check for sampled days
        self.sampled_days = sampled_days if sampled_days > 0 else 1
        if self.sampled_days > 1:
            self._sample_days()
        ...
        for agent in self.agents:
            ev_action_space = gym.spaces.Dict({
                'action_value': gym.spaces.Box(low=0.0,
                                               ...
                                               shape=(1,)),
                'action_indicator': gym.spaces.Discrete(2),
            })
    ...
    def _sample_days(self):

        # Repeat energy price for the sampled days
        if self.energy_price is not None:
            self.energy_price = np.tile(self.energy_price, self.sampled_days)
        else:
            self.energy_price = np.random.normal(loc=0.05, scale=0.01,
                                                 size=self.sampled_days * 24).round(2)

        # Call the sample method for each EV
        for ev in self.evs:

            ev.value = np.tile(ev.value, self.sampled_days)
            ev.charge = np.tile(ev.charge, self.sampled_days)
            ev.discharge = np.tile(ev.discharge, self.sampled_days)

            # Create placeholders for the sampled days
            temp_connected = None
            temp_discharge = None
            temp_charge = None
            temp_requirement_soc = None
            temp_arrival_soc = None

            for i in range(self.sampled_days):
                n_trips, schedule_connected, \
                    schedule_discharge, schedule_charge, \
                    schedule_requirement_soc, schedule_arrival_soc = ev.sample()

                # Concatenate the sampled days
                if temp_connected is None:
                    temp_connected = schedule_connected
                    temp_discharge = schedule_discharge
                    temp_charge = schedule_charge
                    temp_requirement_soc = schedule_requirement_soc
                    temp_arrival_soc = schedule_arrival_soc
                else:
                    temp_connected = np.hstack((temp_connected, schedule_connected))
                    temp_discharge = np.hstack((temp_discharge, schedule_discharge))
                    temp_charge = np.hstack((temp_charge, schedule_charge))
                    temp_requirement_soc = np.hstack((temp_requirement_soc, schedule_requirement_soc))
                    temp_arrival_soc = np.hstack((temp_arrival_soc, schedule_arrival_soc))

            # Set the sampled days
            ev.schedule_connected = temp_connected
            ev.schedule_discharge = temp_discharge
            ev.schedule_charge = temp_charge
            ev.schedule_requirement_soc = temp_requirement_soc * ev.capacity_max
            ev.schedule_arrival_soc = temp_arrival_soc * ev.capacity_max
    ...
    def _do_action(self, ev, action) -> (float, float, float, float, float, float):
        """
        ...
        if action_indicator == 0:
            ...
            raise ValueError('Action indicator should be 0 or 1')

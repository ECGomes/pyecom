...
from src.priorities import RandomPriorityV0
...
class EnergyCommunityRandomPriorityV0(MultiAgentEnv):
    """
    Based on a random priority, this environment is designed to
    simulate an energy community with multiple agents.
    """

    metadata = {'name': 'EnergyCommunityRandomPriority-v0'}
    ...
    def _reset(self):
        ...
        if self.timestep == 0:
            ...
            priority_names = np.array([x.name for x in self.storages + self.evs])

            # Define the execution order
            self.priority_system = RandomPriorityV0(priority_names,
                                                    seed=self.seed)
        ...
        priorities_for_timestep = self.priority_system.priorities[-1].tolist()
        self.execution_order = np.append(priorities_for_timestep, 'aggregator')
        ...
        for storage in self.storages:
            storage_observations[storage.name] = gym.spaces.Dict({
                ...
                'predicted_consumption': gym.spaces.Box(low=0, high=99999.0, shape=(self.look_ahead,),
                                                        dtype=np.float32),
                'priority_score': gym.spaces.Box(low=-99999.0, high=99999.0, shape=(1,), dtype=np.int32)
            })
    ...
    def __get_storage_observations__(self, storage) -> dict:
        ...
        current_priority = self.priority_system.priorities[-1].tolist().index(storage.name)
    ...
    def __get_ev_observations__(self, ev) -> dict:
        ...
        current_priority = self.priority_system.priorities[-1].tolist().index(ev.name)
            ...
            if self.execution_order[self._current_agent_idx] not in action_dict:
                ...
                if all(self.executed_agents):
                    ...
                    if self.timestep >= self.max_timestep:
                        ...
                        # Calculate the next step priorities
                        self.priority_system.update_priority()
                        priorities_for_timestep = self.priority_system.priorities[-1].tolist()
                        self.execution_order = np.append(
                            priorities_for_timestep, 'aggregator'
                        )

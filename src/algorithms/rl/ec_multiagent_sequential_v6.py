...
class EnergyCommunitySequentialV6(MultiAgentEnv):
    ...
    def __execute_storage_actions__(self, storage, actions) -> tuple[float, float]:
        ...
        if storage_action == 0.0:
            ...
            # Check if the storage values have been altered during the last 24 hours
            if self.timestep >= 23:
                # Check if the storage has been charged or discharged
                if (np.sum(storage.charge[self.timestep-23:self.timestep]) > 0) \
                        | (np.sum(storage.discharge[self.timestep-23:self.timestep]) > 0):
                ...
                if self.available_ren_energy > 0:
                    ...
                    if to_charge >= self.available_ren_energy:
                        ...
                        cost = (to_charge - ren_charge) * \
                               ...
                               ren_charge * (storage.cost_charge[self.timestep] - self.aggregator.import_cost[self.timestep])

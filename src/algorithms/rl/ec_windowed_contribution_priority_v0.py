...
class EnergyCommunityWindowedContributionPriorityV0(MultiAgentEnv):
    """
    ...
    metadata = {'name': 'EnergyCommunityWindowedContribPriority-v0'}
    ...
    def create_contribution_dict(self):
        ...
        contributions = {}
        # Calculate the indexes to use based on the look ahead values
        idx_start = self.timestep - self.look_ahead if self.timestep >= self.look_ahead else 0

        for res in self.storages:
            contributions[res.name] = {'generation': np.sum(res.discharge[idx_start:self.timestep]),
                                       'consumption': np.sum(res.charge[idx_start:self.timestep])}
        for res in self.evs:
            contributions[res.name] = {'generation': np.sum(res.discharge[idx_start:self.timestep]),
                                       'consumption': np.sum(res.charge[idx_start:self.timestep])}

...
class EnergyCommunityBaselineV5(MultiAgentEnv):
    """
    ...
    def _reset(self,
        ...
        self.ren_generation: float = np.sum([gen.upper_bound for gen in self.ren_generators], axis=0)

        # Timestep counter
        self.timestep: int = 0

        # Available overall and renewable energy for current timestep
        self.available_energy: float = -self.load_consumption[self.timestep] + self.ren_generation[self.timestep]
    ...
    def __execute_storage_actions__(self, storage, actions) -> tuple[float, float]:
        ...
        # Charge state
            ...
            cost = to_charge * 0.0  # storage.cost_charge[self.timestep]
        ...
        # Discharge state
            ...
            # Calculate the cost
            cost = to_discharge * 0.0  # storage.cost_discharge[self.timestep]
    ...
    def __execute_ev_actions__(self, ev, actions) -> tuple[float, float]:
        ...
        # Else the EV is connected
            ...
            # Charge state
                ...
                # Calculate the cost
                cost += charge * ev.schedule_charge[self.timestep] * 0.0  # ev.cost_charge[self.timestep]
            ...
            # Discharge state
                ...
                # Calculate the cost
                cost += discharge * ev.schedule_discharge[self.timestep] * 0.0  # ev.cost_discharge[self.timestep]

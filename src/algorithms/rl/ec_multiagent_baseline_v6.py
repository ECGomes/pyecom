...
class EnergyCommunityBaselineV6(MultiAgentEnv):
    """
    ...
    V4:
    - Removed the possibility of controlling the renewable generators

    V5:
    - Removed renewable generators as an agent.
    - They are now part of the environment and full production is always assumed.
    """
    ...
    def _reset(self):
        ...
        # Create the agents
        self.possible_agents = ['storage', 'ev', 'gen']
        self.agents = self.__create_agents__()
        self._agent_ids = set(self.agents)
        self.terminateds = set()
        self.truncateds = set()

        # Reset episode costs and penalties
    ...
    def step(self, action_dict: dict) -> tuple:
        ...
        if len(action_dict) > 0:
            ...
            # Set the aggregator to last
            new_action_dict: list = [x for x in action_dict.keys() if 'aggregator' not in x]
            new_action_dict.append('aggregator')
    ...
    def _get_observations(self):
        ...
        observations['aggregator'] = self.__get_aggregator_observations__()

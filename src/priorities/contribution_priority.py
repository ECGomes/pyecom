...
class DiscountedContributionPriority(Priority):
    """
    Calculate the priority of resources based on their discounted contribution to the system.
    The contribution is calculated as the fraction of the resource's contribution to the
    total system consumption and total system generation.
    ...
    def update_resources(self, new_data: dict, timestep: int):
        ...
        if timestep == 1:
            ...
            self.priorities.loc[timestep, :] = self.discount * self.priorities.loc[timestep-1, :].values \
                                               + df.loc['priorities', :].values

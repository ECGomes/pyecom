...
from src.algorithms.rl import EnergyCommunityContributionPriorityV5
...
if SEASON == 'winter':
    ...
    SEASON_END = winter_end - 1344  # Remove the last two weeks for testing purposes. 14 days * 96 steps = 1344
elif SEASON == 'spring':
...
temp_env = EnergyCommunityContributionPriorityV5(ren_generators=generators,
                                        generators=[],
                                        loads=loads,
                                        storages=storages,
                                        evs=evs,
...
env = EnergyCommunityContributionPriorityV5(ren_generators=generators,
                                   generators=[],
                                   loads=loads,
                                   storages=storages,
                                   evs=evs,
                                   ...
                                   max_episode_length=N_STEPS,
                                   seed=42)
register_env("EC_Contrib_V5", lambda config: env)

# Define the PPOConfig
_config = (PPOConfig()
           .environment(env="EC_Contrib_V5", disable_env_checking=False)
